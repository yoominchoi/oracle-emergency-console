...
def init_db():
    ...
    DECLARE 
        seq_exists NUMBER;
    BEGIN
        SELECT COUNT(*) INTO seq_exists FROM all_sequences WHERE sequence_name = 'INCIDENT_ID_SEQ';
        IF seq_exists = 0 THEN
            EXECUTE IMMEDIATE 'CREATE SEQUENCE incident_id_seq START WITH 1 INCREMENT BY 1 NOCACHE';
        END IF;
    END;
    ''')
    ...
    DECLARE 
        trig_exists NUMBER;
    BEGIN
        SELECT COUNT(*) INTO trig_exists FROM all_triggers WHERE trigger_name = 'INCIDENT_ID_TRIGGER';
        IF trig_exists = 0 THEN
            EXECUTE IMMEDIATE '
                CREATE OR REPLACE TRIGGER incident_id_trigger
                BEFORE INSERT ON incidents
                FOR EACH ROW
                WHEN (NEW.incident_id IS NULL)
                BEGIN
                  SELECT incident_id_seq.NEXTVAL
